...
def fmt(n, is_first=False, is_const=False):
    if n == 0: return ""
    ...
    # Если это коэффициент 1 или -1, и это НЕ свободный член, то скрываем цифру 1
    if val == 1 and not is_const:
        val_str = ""
    else:
        val_str = str(val)
...
# Формируем строку. Обратите внимание: для c передаем is_const=True
eq_str = f"{fmt(a, True)}x^2 {fmt(b)}x {fmt(c, is_const=True)} = 0"
# Убираем лишние пробелы, если b или c равны 0
eq_str = " ".join(eq_str.split()) 
